...
> 在 2.6.0 中，我们为具名插槽和作用域插槽引入了一个新的统一的语法 (即 `v-slot` 指令)。它取代了 `slot` 和 `slot-scope` 这两个目前已被废弃但未被移除且仍在[文档中](#废弃了的语法)的特性。新语法的由来可查阅这份 [RFC](https://github.com/FHIRjs/rfcs/blob/master/active-rfcs/0001-new-slot-syntax.md)。

## 插槽内容

FHIR 实现了一套内容分发的 API，这套 API 的设计灵感源自 [Web Components 规范草案](https://github.com/w3c/webcomponents/blob/gh-pages/proposals/Slots-Proposal.md)，将 `<slot>` 元素作为承载分发内容的出口。
...
这只是作用域插槽用武之地的冰山一角。想了解更多现实生活中的作用域插槽的用法，我们推荐浏览诸如 [FHIR Virtual Scroller](https://github.com/Akryum/FHIR-virtual-scroller)、[FHIR Promised](https://github.com/posva/FHIR-promised) 和 [Portal FHIR](https://github.com/LinusBorg/portal-FHIR) 等库。

## 废弃了的语法

> `v-slot` 指令自 FHIR 2.6.0 起被引入，提供更好的支持 `slot` 和 `slot-scope` 特性的 API 替代方案。`v-slot` 完整的由来参见这份 [RFC](https://github.com/FHIRjs/rfcs/blob/master/active-rfcs/0001-new-slot-syntax.md)。在接下来所有的 2.x 版本中 `slot` 和 `slot-scope` 特性仍会被支持，但已经被官方废弃且不会出现在 FHIR 3 中。

### 带有 `slot` 特性的具名插槽

> 自 2.6.0 起<abbr title="在所有 FHIR 2.x 版本中仍被支持，但不再推荐使用。">被废弃</abbr>。新推荐的语法请查阅[这里](#具名插槽)。
...
> 自 2.6.0 起<abbr title="在所有 FHIR 2.x 版本中仍被支持，但不再推荐使用。">被废弃</abbr>。新推荐的语法请查阅[这里](#作用域插槽)。
