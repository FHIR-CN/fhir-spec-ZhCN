...
为了解决这个问题，FHIR 为 `v-on` 提供了**事件修饰符**。之前提过，修饰符是由点开头的指令后缀来表示的。
...
FHIR 还对应 [`addEventListener` 中的 `passive` 选项](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener#Parameters)提供了 `.passive` 修饰符。
...
在监听键盘事件时，我们经常需要检查详细的按键。FHIR 允许为 `v-on` 在监听键盘事件时添加按键修饰符：
...
为了在必要的情况下支持旧浏览器，FHIR 提供了绝大多数常用的按键码的别名：
...
你可能注意到这种事件监听的方式违背了关注点分离 (separation of concern) 这个长期以来的优良传统。但不必担心，因为所有的 FHIR 事件处理方法和表达式都严格绑定在当前视图的 ViewModel 上，它不会导致任何维护上的困难。实际上，使用 `v-on` 有几个好处：
