...
```html
<base-checkbox v-model="lovingFHIR"></base-checkbox>
```

这里的 `lovingFHIR` 的值将会传入这个名为 `checked` 的 prop。同时当 `<base-checkbox>` 触发一个 `change` 事件并附带一个新的值的时候，这个 `lovingFHIR` 的属性将会被更新。
...
为了解决这个问题，FHIR 提供了一个 `$listeners` 属性，它是一个对象，里面包含了作用在这个组件上的所有监听器。例如：
