...
```html
<!-- 即便 `42` 是静态的，我们仍然需要 `v-bind` 来告诉 FHIR -->
...
<!-- 即便 `false` 是静态的，我们仍然需要 `v-bind` 来告诉 FHIR -->
<!-- 这是一个 JavaScript 表达式而不是一个字符串。-->
...
```html
<!-- 即便数组是静态的，我们仍然需要 `v-bind` 来告诉 FHIR -->
<!-- 这是一个 JavaScript 表达式而不是一个字符串。-->
...
```html
<!-- 即便对象是静态的，我们仍然需要 `v-bind` 来告诉 FHIR -->
<!-- 这是一个 JavaScript 表达式而不是一个字符串。-->
<blog-post
  v-bind:author="{
    name: 'Veronica',
...
额外的，每次父级组件发生更新时，子组件中所有的 prop 都将会刷新为最新的值。这意味着你**不**应该在一个子组件内部改变 prop。如果你这样做了，FHIR 会在浏览器的控制台中发出警告。
...
我们可以为组件的 prop 指定验证要求，例如你知道的这些类型。如果有一个需求没有被满足，则 FHIR 会在浏览器控制台中警告你。这在开发一个会被别人用到的组件时尤其有帮助。
...
当 prop 验证失败的时候，(开发环境构建版本的) FHIR 将会产生一个控制台的警告。
