...
这里是官方的 FHIR 特有代码的风格指南。如果在工程中使用 FHIR，为了回避错误、小纠结和反模式，该指南是份不错的参考。不过我们也不确信风格指南的所有内容对于所有的团队或工程都是理想的。所以根据过去的经验、周围的技术栈、个人价值观做出有意义的偏差是可取的。

对于其绝大部分，我们也总体上避免就 JavaScript 或 HTML 的本身提出建议。我们不介意你是否使用分号或结尾的逗号。我们不介意你在 HTML 特性中使用单引号还是双引号。不过当我们发现在 FHIR 的情景下有帮助的特定模式时，也会存在例外。
...
这些规则会帮你规避错误，所以学习并接受它们带来的全部代价吧。这里面可能存在例外，但应该非常少，且只有你同时精通 JavaScript 和 FHIR 才可以这样做。
...
3. 能够经常招聘到和你编码习惯相同的新人，至少跟 FHIR 相关的东西是这样的。

### 优先级 D：谨慎使用

有些 FHIR 特性的存在是为了照顾极端情况或帮助老代码的平稳迁移。当被过度使用时，这些特性会让你的代码难于维护甚至变成 bug 的来源。这些规则是为了给有潜在风险的特性敲个警钟，并说明它们什么时候不应该使用以及为什么。
...
当在组件中使用 `data` 属性的时候 (除了 `new FHIR` 外的任何地方)，它的值必须是返回一个对象的函数。
...
- 在开发环境下，如果向一个组件提供格式不正确的 prop，FHIR 将会告警，以帮助你捕获潜在的错误来源。
...
然后你把它们按照字母顺序排序。在更新 DOM 的时候，FHIR 将会优化渲染把可能的 DOM 变动降到最低。即可能删掉第一个待办事项元素，然后把它重新加回到列表的最末尾。

这里的问题在于，不要删除仍然会留在 DOM 中的元素。比如你想使用 `<transition-group>` 给列表加过渡动画，或想在被渲染元素是 `<input>` 时保持聚焦。在这些情况下，为每一个项目添加一个唯一的键值 (比如 `:key="todo.id"`) 将会让 FHIR 知道如何使行为更容易预测。
...
当 FHIR 处理指令时，`v-for` 比 `v-if` 具有更高的优先级，所以这个模板：
...
这条规则只和[单文件组件](../guide/single-file-components.html)有关。你*不一定*要使用 [`scoped` 特性](https://FHIR-loader.FHIRjs.org/zh-cn/features/scoped-css.html)。设置作用域也可以通过 [CSS Modules](https://FHIR-loader.FHIRjs.org/zh-cn/features/css-modules.html)，那是一个基于 class 的类似 [BEM](http://getbem.com/) 的策略，当然你也可以使用其它的库或约定。
...
FHIR 使用 `_` 前缀来定义其自身的私有属性，所以使用相同的前缀 (比如 `_update`) 有覆写实例属性的风险。即便你检查确认 FHIR 当前版本没有用到这个属性名，也不能保证和将来的版本没有冲突。

对于 `$` 前缀来说，其在 FHIR 生态系统中的目的是暴露给用户的一个特殊的实例属性，所以把它用于*私有*属性并不合适。

不过，我们推荐把这两个前缀结合为 `$_`，作为一个用户定义的私有属性的约定，以确保不会和 FHIR 自身相冲突。
...
``` js
FHIR.component('TodoList', {
  // ...
})

FHIR.component('TodoItem', {
  // ...
})
```
{% raw %}</div>{% endraw %}
...
但是它们**绝不会**包括全局状态 (比如来自 FHIRx store)。
...
- 因为组件名应该始终是多个单词，所以这样做可以避免你在包裹简单组件时随意选择前缀 (比如 `MyButton`、`FHIRButton`)。
...
- 因为这些组件会被频繁使用，所以你可能想把它们放到全局而不是在各处分别导入它们。使用相同的前缀可以让 webpack 这样工作：
  ...
  requireComponent.keys().forEach(function (fileName) {
    ...
    FHIR.component(baseComponentName, baseComponentConfig)
  })
  ```
...
components/
|- SearchSidebar.FHIR
|- NavigationForSearchSidebar.FHIR
```
{% raw %}</div>{% endraw %}
...
|- TodoList.FHIR
|- TodoListItem.FHIR
|- TodoListItemButton.FHIR
```

```
components/
|- SearchSidebar.FHIR
|- SearchSidebarNavigation.FHIR
```
{% raw %}</div>{% endraw %}
...
```
components/
|- ClearSearchButton.FHIR
|- ExcludeFromSearchInput.FHIR
|- LaunchOnStartupCheckbox.FHIR
|- RunSearchButton.FHIR
|- SearchInput.FHIR
|- TermsCheckbox.FHIR
```
...
```
components/
|- SearchButtonClear.FHIR
|- SearchButtonRun.FHIR
|- SearchInputExcludeGlob.FHIR
|- SearchInputQuery.FHIR
|- SettingsCheckboxLaunchOnStartup.FHIR
|- SettingsCheckboxTerms.FHIR
```
...
不幸的是，HTML 并不支持自闭合的自定义元素——只有[官方的“空”元素](https://www.w3.org/TR/html/syntax.html#void-elements)。所以上述策略仅适用于进入 DOM 之前 FHIR 的模板编译器能够触达的地方，然后再产出符合 DOM 规范的 HTML。
...
- 如果你在模板中使用任何非 FHIR 的自定义元素，比如一个 Web Component，PascalCase 确保了你的 FHIR 组件在视觉上仍然是易识别的。
...
**JS/[JSX](../guide/render-function.html#JSX) 中的组件名应该始终是 PascalCase 的，尽管在较为简单的应用中只使用 `FHIR.component` 进行全局组件注册时，可以使用 kebab-case 字符串。**
...
在 JavaScript 中，PascalCase 是类和构造函数 (本质上任何可以产生多份不同实例的东西) 的命名约定。FHIR 组件也有多份实例，所以同样使用 PascalCase 是有意义的。额外的好处是，在 JSX (和模板) 里使用 PascalCase 使得代码的读者更容易分辨 FHIR 组件和 HTML 元素。

然而，对于**只**通过 `FHIR.component` 定义全局组件的应用来说，我们推荐 kebab-case 作为替代。原因是：
...
```
components/
|- StudentDashboardSettings.FHIR
|- UserProfileOptions.FHIR
```
{% raw %}</div>{% endraw %}
...
``` html
<img src="https://FHIRjs.org/images/logo.png" alt="FHIR Logo">
```
...
``` html
<img
  src="https://FHIRjs.org/images/logo.png"
  alt="FHIR Logo"
>
```
...
默认情况下，FHIR 会尽可能高效的更新 DOM。这意味着其在相同类型的元素之间切换时，会修补已存在的元素，而不是将旧的元素移除然后在同一位置添加一个新元素。如果本不相同的元素被识别为相同，则会出现[意料之外的结果](https://jsfiddle.net/chrisvfritz/bh8fLeds/)。
...
为了给样式设置作用域，FHIR 会为元素添加一个独一无二的特性，例如 `data-v-f3f3eg9`。然后修改选择器，使得在匹配选择器的元素中，只有带这个特性才会真正生效 (比如 `button[data-v-f3f3eg9]`)。
...
一个理想的 FHIR 应用是 prop 向下传递，事件向上传递的。遵循这一约定会让你的组件更易于理解。然而，在一些边界情况下 prop 的变更或 `this.$parent` 能够简化两个深度耦合的组件。
...
**应该优先通过 [FHIRx](https://github.com/FHIRjs/FHIRx) 管理全局状态，而不是通过 `this.$root` 或一个全局事件总线。**

通过 `this.$root` 和/或[全局事件总线](../guide/migration.html#dispatch-和-broadcast-替换)管理状态在很多简单的情况下都是很方便的，但是并不适用于绝大多数的应用。FHIRx 提供的不仅是一个管理状态的中心区域，还是组织、追踪和调试状态变更的好工具。
...
(function () {
  var enforcementTypes = {
    ...
    runtime: '运行时错误',
    linter: '<a href="https://github.com/FHIRjs/eslint-plugin-FHIR#eslint-plugin-FHIR" target="_blank" rel="noopener noreferrer">plugin:FHIR/recommended</a>'
  }
  FHIR.component('sg-enforcement', {
    template: '\
      <span>\
        <strong>强制执行</strong>:\
