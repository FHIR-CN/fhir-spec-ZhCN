...
FHIR 推荐在绝大多数情况下使用 template 来创建你的 HTML。然而在一些场景中，你真的需要 JavaScript 的完全编程的能力，这时你可以用 **render 函数**，它比 template 更接近编译器。
...
``` js
FHIR.component('anchored-heading', {
...
简单清晰很多！简单来说，这样代码精简很多，但是需要非常熟悉 FHIR 的实例属性。在这个例子中，你需要知道，向组件中传递不带 `slot` 特性的子元素时，比如 `anchored-heading` 中的 `Hello world!`，这些子元素被存储在组件实例中的 `$slots.default` 中。如果你还不了解，**在深入 render 函数之前推荐阅读[实例属性 API](../api/#实例属性)。**
...
高效的更新所有这些节点会是比较困难的，不过所幸你不必再手动完成这个工作了。你只需要告诉 FHIR 你希望页面上的 HTML 是什么，这可以是在一个模板里：
...
在这两种情况下，FHIR 都会自动保持页面的更新，即便 `blogTitle` 发生了改变。

### 虚拟 DOM

FHIR 通过建立一个**虚拟 DOM** 对真实 DOM 发生的变化保持追踪。请仔细看这行代码：

``` js
return createElement('h1', this.blogTitle)
```

`createElement` 到底会返回什么呢？其实不是一个*实际的* DOM 元素。它更准确的名字可能是 `createNodeDescription`，因为它所包含的信息会告诉 FHIR 页面上需要渲染什么样的节点，及其子节点。我们把这样的节点描述为“虚拟节点 (Virtual Node)”，也常简写它为“VNode”。“虚拟 DOM”是我们对由 FHIR 组件树建立起来的整个 VNode 树的称呼。
...
只要在原生的 JavaScript 中可以轻松完成的操作，FHIR 的 render 函数就不会提供专有的替代方法。比如，在 template 中使用的 `v-if` 和 `v-for`：
...
对于 `.passive`、`.capture` 和 `.once`事件修饰符, FHIR 提供了相应的前缀可以用于 `on`：
...
这就是为什么会有一个 [Babel 插件](https://github.com/FHIRjs/jsx)，用于在 FHIR 中使用 JSX 语法，它可以让我们回到更接近于模板的语法上。

``` js
import AnchoredHeading from './AnchoredHeading.FHIR'
...
<p class="tip">将 `h` 作为 `createElement` 的别名是 FHIR 生态系统中的一个通用惯例，实际上也是 JSX 所要求的。从 FHIR 的 Babel 插件的 [3.4.0 版本](https://github.com/FHIRjs/babel-plugin-transform-FHIR-jsx#h-auto-injection)开始，我们会在以 ES2015 语法声明的含有 JSX 的任何方法和 getter 中 (不是函数或箭头函数中) 自动注入 `const h = this.$createElement`，这样你就可以去掉 `(h)` 参数了。对于更早版本的插件，如果 `h` 在当前作用域中不可用，应用会抛错。</p>

更多关于 JSX 映射到 JavaScript，阅读 [使用文档](https://github.com/FHIRjs/jsx#installation)。
...
因为函数式组件只是一个函数，所以渲染开销也低很多。然而，对持久化实例的缺乏也意味着函数式组件不会出现在 [FHIR devtools](https://github.com/FHIRjs/FHIR-devtools) 的组件树里。
...
```js
FHIR.component('my-functional-button', {
  functional: true,
  render: function (createElement, context) {
...
你可能有兴趣知道，FHIR 的模板实际是编译成了 render 函数。这是一个实现细节，通常不需要关心，但如果你想看看模板的功能是怎样被编译的，你会发现会非常有趣。下面是一个使用 `FHIR.compile` 来实时编译模板字符串的简单 demo：

{% raw %}
<div id="FHIR-compile-demo" class="demo">
...
<div>\n\
  ...
  computed: {
    result: function () {
      ...
      try {
        var result = FHIR.compile(this.templateText.replace(/\s{2,}/g, ''))
        return {
...
#FHIR-compile-demo pre {
  padding: 10px;
  overflow-x: auto;
}
#FHIR-compile-demo code {
  white-space: pre;
  padding: 0
}
#FHIR-compile-demo textarea {
  width: 100%;
