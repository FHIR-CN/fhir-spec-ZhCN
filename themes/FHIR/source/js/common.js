(function () {
  ...
  function initApiSpecLinks () {
    ...
    function createSourceSearchPath (query) {
      query = query
        .replace(/\([^\)]*?\)/g, '')
        .replace(/(FHIR\.)(\w+)/g, '$1$2" OR "$2')
        .replace(/vm\./g, 'FHIR.prototype.')
      return 'https://github.com/search?utf8=%E2%9C%93&q=repo%3AFHIRjs%2FFHIR+extension%3Ajs+' + encodeURIComponent('"' + query + '"') + '&type=Code'
    }
  }
